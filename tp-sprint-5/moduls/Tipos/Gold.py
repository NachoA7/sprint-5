...
class Gold:
    def __init__(self, nombre, apellido, caja_ahorro_dolares = False) -> None:
        ...
        self.__tarjeta_debito = f"{randint(2000, 2999)} {randint(1111, 9999)} {randint(1111, 9999)} {randint(1111, 9999)}"
        self.__cajas_de_ahorro_pesos = [[1, 0]]
        self.__cuenta_corriente = [1, 0]
        ...
        if caja_ahorro_dolares:
            self.__cajas_de_ahorro_dolares = [[1, 0]]
    ...
    def agregarCajaEnDolares(self):
        try:
            if len(self.__cajas_de_ahorro_dolares) < 2:
                self.__cajas_de_ahorro_dolares.append([f"dolares{len(self.__cajas_de_ahorro_dolares) + 1}", len(self.__cajas_de_ahorro_dolares) + 1, 0])
            else:
                raise MaxCajaAhorro()
        except MaxCajaAhorro as e:
            print("Error: ", str(e))
